...
export default function Analysis({ code, question }: AnalysisProps) {
  ...
  const analyzeCode = async () => {
    ...
    const cleanedCode = code
      ...
      .replace(/\r/g, "")
      .trim();
    console.log(`Code after Cleaning: \n ${cleanedCode}`);
    try {
      const prompt = `You are a LeetCode assistant. Given the question and user's Python code, follow these rules:
...
${cleanedCode}`;
      ...
      setImprovements(improvementParts.join("Suggested Improvements:").trim());
    } catch (err) {
